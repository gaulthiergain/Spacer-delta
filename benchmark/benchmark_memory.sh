...
compute_memory(){
    ...
    if [ "$method" == "dce" ]; then
        ...
        uk_config="uk_config_spacer"
		./copy_shm.sh 0 spacer $TEST
	elif [ "$method" == "delta-slt" ]; then
        firecracker_bin="firecracker${release}"
        uk_config="uk_config_delta"
		./copy_shm.sh 0 delta $TEST
    fi
    ...
    for i in $(seq 1 $len)
    do
        ${firecracker_bin} --no-api --config-file "${UNIKRAFT_APPS}@v${i}/${uk_config}.json" &> "/tmp/instance${i}.log" &
        sleep 10
    done
...
do_benchmark(){
    ...
    for method in "${array_method[@]}"; do
        ...
        compute_memory "${RESULTDIR}/memory_$1/${method}.csv" "$method"
    done
}
